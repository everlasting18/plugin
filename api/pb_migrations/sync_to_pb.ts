 ...
 *    (1_licenses.json → 2_usage.json → 3_usage_logs.json)
 * 2. Lấy API Token: Settings → API preview → copy token (eyJ...)
 * 3. Paste token vào biến PB_TOKEN bên dưới HOẶC chạy với env:
 *    PB_TOKEN=eyJ... deno run --allow-net pb_migrations/sync_to_pb.ts
 */
...
// Nếu không có env token, hardcode tạm (thay bằng token thật)
if (!PB_TOKEN) {
  console.warn("⚠️  PB_TOKEN not set via env. Set it with:");
  console.warn("    PB_TOKEN=eyJ... deno run --allow-net pb_migrations/sync_to_pb.ts");
  console.warn("    Or edit this file and set PB_TOKEN constant directly.\n");
}

const TOKEN = PB_TOKEN || Deno.args[0] || "";
...
async function main() {
  ...
  if (!TOKEN) {
    console.error("❌ Missing PB_TOKEN. Get it from: Settings → API preview");
    console.error("   Then run: PB_TOKEN=eyJ... deno run --allow-net pb_migrations/sync_to_pb.ts");
    Deno.exit(1);
  }
  ...
  try {
    ...
    if (msg.includes("401") || msg.includes("403")) {
      ...
      console.error("❌ 'licenses' collection not found. Import it first via dashboard:");
      console.error("   Settings → Import collections → paste 1_licenses.json");
    } else {
